...
class MenuHandlerBase:
    ...
    def _apply_shortcut(self, action, command_group, command_name):
        r"""Display shortcut text in menu label without registering a conflicting QAction shortcut.

        Retrieves the shortcut string from GlobalShortcutManager and appends it
        as a tab-separated suffix to the action label (e.g. "Open Directory\tCtrl+O").
        This avoids conflicts with QShortcut instances registered by GlobalShortcutManager.

        Args:
            action (QAction): The menu action to update.
            command_group (str): Name of the command group (e.g., "file_commands").
            command_name (str): Specific command name within the group.
        """
        shortcut_key = self.main_window.shortcut_manager.get_shortcut_for_command(
            command_group, command_name
        )
        logger.debug(
            "get_shortcut_for_command('%s', '%s') = '%s'",
            command_group,
            command_name,
            shortcut_key,
        )
...
class ViewMenuHandler(MenuHandlerBase):
    ...
    def __init__(self, main_window):
        """Initialize ViewMenuHandler with reference to the main window."""
        self.main_window = main_window
...
class AudioMenuHandler(MenuHandlerBase):
    ...
    def __init__(self, main_window):
        """Initialize AudioMenuHandler with reference to the main window."""
        self.main_window = main_window
...
class AnalysisMenuHandler(MenuHandlerBase):
    ...
    def __init__(self, main_window):
        """Initialize AnalysisMenuHandler with reference to the main window."""
        self.main_window = main_window
...
class HelpMenuHandler(MenuHandlerBase):
    ...
    def __init__(self, main_window):
        """Initialize HelpMenuHandler with reference to the main window."""
        self.main_window = main_window
